...
class SimModuleNode final : public ModuleNode {
 public:
  SimModuleNode(LoweredFunc func, std::string host_code,
                std::vector<std::string> arg_names, std::string dev_code,
                std::string cfg_code, std::string platform,
  ...
  const char* type_key() const { return "unified_sim"; }
  ...
  PackedFunc GetFunction(
    ...
    return PackedFunc([this](TVMArgs args, TVMRetValue* rv) {
      ...
      if (shmids.size() > 0) {
        ...
        CHECK(shmids.size() == (unsigned)args.size()) << "invalid inputs";
        ...
        GenHostCode(args, shmids, arg_types, func_, platform_, host_,
                    arg_names_, empty, options_["project"]);
        ...
        if (!pre_compiled) {
          ...
          if (const auto* f = Registry::Get("copy_and_compile")) {
            ...
            (*f)(platform_, mode, backend, empty, cfg_, tcl)
                .
                operator std::string();
          }
        }
...
Module CreateSimModule(LoweredFunc func, std::string host_code,
                       std::string dev_code, std::string cfg_code,
                       std::vector<std::string> arg_names, std::string platform,
                       std::unordered_map<std::string, std::string> options) {
  std::shared_ptr<SimModuleNode> n = std::make_shared<SimModuleNode>(
      func, host_code, arg_names, dev_code, cfg_code, platform, options);
  return Module(n);
}
}  // namespace runtime
...
runtime::Module BuildSimModule(Array<LoweredFunc> funcs, Array<Expr> attrs,
  ...
  return runtime::CreateSimModule(funcs[0], cg_host.GetHost(),
                                  cg_dev.GetDevice(), cg_host.GetConfig(),
                                  cg_host.arg_names, platform, options);
}
...
TVM_REGISTER_API("codegen.build_sim")
    .set_body([](TVMArgs args, TVMRetValue* rv) {
      ...
      if (type == "rocket") {
        *rv = BuildSimModule<CodeGenRV64PPAC, CodeGenRV64PPAC>(args[0], args[1],
                                                               args[2]);

      } else if (type == "sdaccel" || type == "vitis") {
        if (lang == "xocl") {
          *rv = BuildSimModule<CodeGenXOCLHost, CodeGenXOCL>(args[0], args[1],
                                                             args[2]);
        } else if (lang == "vhls") {
          *rv = BuildSimModule<CodeGenXOCLHost, CodeGenVivadoHLS>(
              args[0], args[1], args[2]);
        } else {
          LOG(FATAL) << "sdaccel does not support " << lang << " backend";
        }
        ...
        *rv = BuildSimModule<CodeGenVivadoHLS, CodeGenVivadoHLS>(
        ...
        if (lang == "aocl") {
          *rv = BuildSimModule<CodeGenAOCLHost, CodeGenAOCL>(args[0], args[1],
                                                             args[2]);
        } else if (lang == "ihls") {
