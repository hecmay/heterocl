...
__kernel void flow_calc(__global uint64_t* restrict flow_calc_output) {
    for (int32_t r = 0; r < 436; ++r) {
      for (int32_t c = 0; c < 1024; ++c) {
        if ((((2 <= r) && (r < 434)) && (2 <= c)) && (c < 1022)) {
          float scalar0;
          scalar0 = ((float)read_channel_intel(c_buf_1));
          float a;
          int8_t _converter;
          _converter(7, 0) = ((uint8_t)((scalar0 >> 8) & ((1L << (0 - 8)) - 1)))(7, 0);
          a = ((float)_converter);
          float b;
          ...
          float d;
          int8_t _converter3;
          _converter3(7, 0) = ((uint8_t)((scalar0 >> 80) & ((1L << (72 - 80)) - 1)))(7, 0);
          d = ((float)_converter3);
          float e;
          ...
          flow_calc_output[(c1 + ((float)(r * 1024)))](31, 0) = _converter6.to;
          union { float from; uint32_t to;} _converter7;
          _converter7.from = ((e * (d - a)) / denom);
          flow_calc_output[(c1 + ((float)(r * 1024)))](63, 32) = _converter7.to;
        }
      }
    }
}
...
__kernel void tensor_weight_x() {
    ...
    for (int32_t y_reuse = 0; y_reuse < 438; ++y_reuse) {
      for (int32_t x = 0; x < 1024; ++x) {
        ...
        tensor_weight_x_tensor_y_reuse[(x + 2048)] = read_channel_intel(c_buf_2);
        if (2 <= y_reuse) {
          float reducer5;
          ...
          write_channel_intel(c_buf_1, (float)((x < 1) ? 0.000000e+00f : reducer5));
        }
      }
    }
}
...
__kernel void tensor_weight_y() {
    ...
    for (int32_t y_reuse = 0; y_reuse < 438; ++y_reuse) {
      for (int32_t x = 0; x < 1024; ++x) {
        ...
        tensor_weight_y_out_product_reuse[(x + 2048)] = read_channel_intel(c_buf_3);
        if (2 <= y_reuse) {
          float reducer4;
          ...
          write_channel_intel(c_buf_2, (float)(((3 <= y_reuse) && (y_reuse < 437)) ? reducer4 : 0.000000e+00f));
        }
      }
    }
}
...
__kernel void outer_product() {
    for (int32_t y = 0; y < 436; ++y) {
      for (int32_t x = 0; x < 1024; ++x) {
        uint144_t temp;
        float t;
        t = read_channel_intel(c_buf_7);
        float a;
        int32_t _converter;
        _converter(31, 0) = ((uint32_t)((t >> 32) & ((1L << (0 - 32)) - 1)))(31, 0);
        a = ((float)_converter);
        float b;
        union { uint32_t from; float to;} _converter1;
...
__kernel void grad_weight_x() {
    ...
    for (int32_t y = 0; y < 436; ++y) {
      for (int32_t x_reuse = 0; x_reuse < 1024; ++x_reuse) {
        ...
        grad_weight_x_y_filt_reuse[6] = read_intel_channel(c_buf_8);
        if (6 <= x_reuse) {
          float reducer3;
          ...
          write_channel_intel(c_buf_7, reducer3);
        }
      }
    }
}
...
__kernel void grad_weight_y() {
    ...
    for (int32_t y_reuse = 0; y_reuse < 436; ++y_reuse) {
      for (int32_t x = 0; x < 1024; ++x) {
        ...
        grad_weight_y_pack_reuse[(x + 6144)] = read_channel_intel(c_buf_3);
        if (6 <= y_reuse) {
          float reducer2;
          ...
          write_channel_intel(c_buf_8, reducer2);
        }
      }
    }
}
...
__kernel void grad_pack() {
    for (int32_t y = 0; y < 436; ++y) {
      for (int32_t x = 0; x < 1024; ++x) {
        ...
        _converter.from = read_channel_intel(c_buf_4);
        t(31, 0) = _converter.to;
        union { float from; uint32_t to;} _converter1;
        _converter1.from = read_channel_intel(c_buf_5);
        t(63, 32) = _converter1.to;
        union { float from; uint32_t to;} _converter2;
        _converter2.from = read_channel_intel(c_buf_6);
        t(95, 64) = _converter2.to;
        write_channel_intel(c_buf_3, t);
      }
    }
}
...
__kernel void calc_z_gradient(__global float* restrict calc_z_gradient_img0, __global float* restrict calc_z_gradient_img1, __global float* restrict calc_z_gradient_img2, __global float* restrict calc_z_gradient_img3, __global float* restrict calc_z_gradient_img4) {
    ...
    for (int32_t y = 0; y < 436; ++y) {
      for (int32_t x = 0; x < 1024; ++x) {
        write_channel_intel(c_buf_6, ((((((calc_z_gradient_img0[(x + (y * 1024))] * ((float)g_w[0])) + (calc_z_gradient_img1[(x + (y * 1024))] * ((float)g_w[1]))) + (calc_z_gradient_img2[(x + (y * 1024))] * ((float)g_w[2]))) + (calc_z_gradient_img3[(x + (y * 1024))] * ((float)g_w[3]))) + (calc_z_gradient_img4[(x + (y * 1024))] * ((float)g_w[4]))) * 8.333334e-02f));
      }
    }
}
    ...
    for (int32_t y_reuse = 0; y_reuse < 436; ++y_reuse) {
      for (int32_t x = 0; x < 1024; ++x) {
        ...
        if (4 <= y_reuse) {
          ...
          write_channel_intel(c_buf_5, reducer1);
        }
      }
    }
}
    ...
    for (int32_t y = 0; y < 436; ++y) {
      for (int32_t x_reuse = 0; x_reuse < 1024; ++x_reuse) {
        ...
        if (4 <= x_reuse) {
          ...
          write_channel_intel(c_buf_4, reducer0);
        }
      }
    }
}
